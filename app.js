...
async function generateSchedule() {
    ...
    while (!_generatorStop) {
        ...
        // ФАЗА 3.5: Day Rebalance
        // Якщо після repair залишились нерозміщені — пробуємо перебалансувати:
        // знаходимо урок який блокує клас/вчителя на потрібному слоті
        // і переміщуємо його в інший день де є місце.
        if (stillUnplaced.length > 0) {
            await dayRebalance(stillUnplaced, schedule, startTime, restart, total);
        }
        ...
        for (const late of lateList) {
            ...
            for (const early of earlyList) {
                if (canSwap(late, early, schedule)) {
                    ...
                    // Перевіряємо що після swap не виникнуть вікна
                    late.slot  = slotEarly;
                    early.slot = slotLate;
                    // Перевірка вікон після swap
                    const pseudoLate = { items: [late], priority: getPriority(late.subject), type: late.pairType || 'single' };
                    const pseudoEarly = { items: [early], priority: getPriority(early.subject), type: early.pairType || 'single' };
                    // Тимчасово прибираємо обидва зі schedule для перевірки
                    const schedWithout = schedule.filter(x => x !== late && x !== early);
                    const lateOk = isHardValid(pseudoLate, late, late.day, late.slot, schedWithout);
...
function canSwap(lessonA, lessonB, schedule) {
    ...
    // Вчитель A зайнятий на слоті B?
    if (schedule.some(ls => ls !== lessonA && ls.day === d && ls.slot === lessonB.slot && ls.teacherId === lessonA.teacherId)) return false;
    // Вчитель B зайнятий на слоті A?
    if (schedule.some(ls => ls !== lessonB && ls.day === d && ls.slot === lessonA.slot && ls.teacherId === lessonB.teacherId)) return false;
    // Red zone
    ...
    let gapCountA=0; for(let i=1;i<tASlotsNew.length;i++) { if(tASlotsNew[i]-tASlotsNew[i-1]>1) gapCountA++; } if(gapCountA>1) return false;
    // Вчитель B на новій позиції
...
async function minConflictsRepair(unplacedTasks, schedule, startTime, restart, total, best) {
    ...
    for (let iter = 0; iter < MAX_REPAIR_ITERS && unplacedTasks.length > 0; iter++) {
        ...
        // Збираємо кандидатні слоти для розміщення через евакуацію блокерів.
        // Слот-кандидат: без red-zone і без NO-GAP порушення для класу,
        // але може бути зайнятий вчителем або класом (їх евакуюємо).
        const candidates = [];
        for (let d = 0; d < 5; d++) {
            for (let s = 1; s <= 7; s++) {
                ...
                // Пропускаємо слоти з > 2 блокерами (занадто складно)
                if (blockers.length > 2) continue;

                // Попередній score (може включати Infinity якщо блокери заважають isHardValid)
                // Використовуємо простий score без isHardValid-перевірки для кандидатів
                const rawScore = s + d * 7; // просто позиційний score для впорядкування
                candidates.push({ d, s, blockers, score: rawScore });
            }
        }

        // Сортуємо: менше блокерів і раніший слот — першим
...
function tryEvacuate(lesson, blockedDay, blockedSlot, schedule, depth) {
    ...
    // Тимчасово видаляємо блокера зі schedule
    schedule.splice(idx, 1);

    // Будуємо фіктивну задачу щоб використати isHardValid
    const pseudoTask = {
        items: [lesson],
    ...
    for (const { d, s } of slots) {
        ...
        if (!teacherConflict && !classConflict) {
            // Слот повністю вільний — розміщуємо
        ...
        // Слот частково зайнятий — каскадна евакуація
        if (depth < 2) {
            const blocker = teacherConflict || classConflict;
            if (!blocker.isManual) {
                const cascadeResult = tryEvacuate(blocker, d, s, schedule, depth + 1);
                if (cascadeResult) {
                    // Перевіряємо що тепер слот справді вільний
...
// =============================================================
// ФАЗА 3.5: DAY REBALANCE
// Використовується коли нерозміщена задача не має жодного
// валідного слоту — тобто evakuация одного уроку недостатня.
//
// Стратегія: для кожного дня де вчитель має вільний слот 1-4(5),
// але клас заповнений — шукаємо урок класу (НЕ від цього вчителя)
// що блокує слот, і переміщуємо його в інший день де той вчитель
// теж має вільний слот і немає конфлікту.
// Це звільняє щілину для нерозміщеного уроку.
// =============================================================
async function dayRebalance(unplacedTasks, schedule, startTime, restart, total) {
    updateLoader(restart, Date.now() - startTime, total - unplacedTasks.length, total, unplacedTasks.length,
        `Рестарт ${restart} | Day Rebalance | Залишилось: ${unplacedTasks.length}`);
    await tick();

    for (let taskIdx = unplacedTasks.length - 1; taskIdx >= 0; taskIdx--) {
        if (_generatorStop) break;
        const task = unplacedTasks[taskIdx];
        const first = task.items[0];
        const maxSlot = task.priority === 1 ? 4 : 5; // priority=1: тільки 1-4, інші: 1-5

        let placed = false;

        // Перебираємо дні
        for (let targetDay = 0; targetDay < 5 && !placed; targetDay++) {
            // Перевіряємо: чи є хоча б один вільний слот 1-maxSlot для вчителя в цей день
            const teacherSlotsDay = new Set(
                schedule.filter(ls => ls.teacherId === first.teacherId && ls.day === targetDay && ls.slot >= 1 && ls.slot <= 7)
                    .map(ls => ls.slot)
            );
            const teacherHasFree = Array.from({length: maxSlot}, (_, i) => i + 1)
                .some(s => !teacherSlotsDay.has(s) && getTeacherStatus(first.teacherId, targetDay, s) !== 2);
            if (!teacherHasFree) continue;

            // Для кожного вільного слоту вчителя 1-maxSlot — перевіряємо що заважає
            for (let targetSlot = 1; targetSlot <= maxSlot && !placed; targetSlot++) {
                if (teacherSlotsDay.has(targetSlot)) continue;
                if (getTeacherStatus(first.teacherId, targetDay, targetSlot) === 2) continue;

                // Знаходимо уроки класу що займають цей слот (блокери)
                const classBlockers = schedule.filter(ls =>
                ...
                if (classBlockers.length === 0) {
                    // Слот вільний і для вчителя і для класу — спробуємо одразу
                    if (isHardValid(task, first, targetDay, targetSlot, schedule)) {
                        commitTask(task, targetDay, targetSlot, schedule);
                        unplacedTasks.splice(taskIdx, 1);
                        placed = true;
                    }
                    continue;
                }
                if (classBlockers.length > 1) continue; // занадто складно

                const blocker = classBlockers[0];
                const blockerTeacherId = blocker.teacherId;

                // Шукаємо куди перемістити блокера: інший день, вільний слот
                // де blocker-вчитель вільний і немає конфліктів для його класу
                const pseudoBlocker = {
                    items: [blocker],
                    priority: getPriority(blocker.subject),
                    type: blocker.pairType || 'single'
                };

                // Тимчасово видаляємо блокера
                const bIdx = schedule.indexOf(blocker);
                if (bIdx === -1) continue;
                schedule.splice(bIdx, 1);

                let movedBlocker = null;

                // Перебираємо альтернативні дні для блокера (не той самий)
                const altDays = Array.from({length: 5}, (_, i) => i).filter(dd => dd !== targetDay);
                // Перемішуємо для різноманіття
                for (let i = altDays.length - 1; i > 0; i--) {
                    const j = Math.floor(Math.random() * (i + 1));
                    [altDays[i], altDays[j]] = [altDays[j], altDays[i]];
                }

                for (const altDay of altDays) {
                    if (movedBlocker) break;
                    // Перевірка доступності вчителя-блокера в цей день
                    if (getTeacherStatus(blockerTeacherId, altDay, 1) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 2) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 3) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 4) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 5) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 6) === 2 &&
                        getTeacherStatus(blockerTeacherId, altDay, 7) === 2) continue; // весь день заборонений

                    for (let altSlot = 1; altSlot <= 7; altSlot++) {
                        if (isHardValid(pseudoBlocker, blocker, altDay, altSlot, schedule)) {
                            // Перевіряємо чи блокер не дублює предмет в цей день для свого класу
                            const sameSubjAltDay = schedule.filter(ls =>
                                ls.classId === blocker.classId &&
                                ls.day === altDay &&
                                ls.subject === blocker.subject
                            ).length;
                            if (sameSubjAltDay >= 1 && getPriority(blocker.subject) <= 2) continue;

                            movedBlocker = { ...blocker, id: 'rb_' + Date.now() + Math.random(), day: altDay, slot: altSlot };
                            schedule.push(movedBlocker);
                            break;
                        }
                    }
                }
                ...
                if (movedBlocker) {
                    // Тепер перевіряємо чи можна поставити нашу задачу
                    if (isHardValid(task, first, targetDay, targetSlot, schedule)) {
                        commitTask(task, targetDay, targetSlot, schedule);
                        unplacedTasks.splice(taskIdx, 1);
                        placed = true;
                    } else {
                        // Не вийшло — відкочуємо переміщення блокера
                        const mIdx = schedule.indexOf(movedBlocker);
                        if (mIdx !== -1) schedule.splice(mIdx, 1);
                        schedule.splice(bIdx, 0, blocker);
                    }
                } else {
                    // Не знайшли куди перемістити блокера — повертаємо
                    schedule.splice(bIdx, 0, blocker);
                }
            }
        }
    ...
    updateLoader(restart, Date.now() - startTime, total - unplacedTasks.length, total, unplacedTasks.length,
        `Рестарт ${restart} | Day Rebalance done | Залишилось: ${unplacedTasks.length}`);
    await tick();
}
...
function buildTasks() {
    ...
    classesIds.forEach(cId => {
        ...
        if (excess <= 0) return;
        // Групуємо задачі по предмету щоб виносити цілі предмети
        const subjectGroups = {};
        tasks.filter(t => t.classId === cId && t.priority > 1).forEach(t => {
            const key = t.items[0].subject + '_' + t.items[0].teacherId;
            if (!subjectGroups[key]) subjectGroups[key] = { tasks: [], priority: t.priority, subject: t.items[0].subject, teacher: state.teachers.find(tt => tt.id === t.items[0].teacherId)?.name || '?' };
            subjectGroups[key].tasks.push(t);
        });
        // Сортуємо групи: спочатку найнижчий пріоритет (3), потім 2
        const groups = Object.values(subjectGroups).sort((a, b) => b.priority - a.priority || a.tasks.length - b.tasks.length);
        for (const group of groups) {
            if (excess <= 0) break;
            // Не виносимо якщо є paired з пріоритетом 1
            if (group.tasks.some(t => t.type !== 'single' && !t.items.every(it => getPriority(it.subject) > 1))) continue;
            // Виносимо стільки задач групи скільки треба для покриття excess
...
function isHardValid(task, first, d, s, schedule) {
    ...
    // Фізкультура — max 1 на день для класу
    if (roomType === 'gym') {
        if (schedule.some(ls => ls.day === d && ls.classId === first.classId &&
            getRoomType(ls.subject) === 'gym' && ls.slot !== s)) return false;
    }
    ...
    //     (мистецтво, музика, фізкультура не дублюються)
    if (task.priority === 3) {
        const sameSubjDay = schedule.filter(ls =>
            ls.day === d && ls.classId === first.classId && ls.subject === first.subject
        ).length;
        if (sameSubjDay >= 1) return false;
    }

    // 11. Пріоритет 2 — max 1 раз на день якщо є інші вільні дні
    if (task.priority === 2) {
        const sameSubjDay = schedule.filter(ls =>
            ls.day === d && ls.classId === first.classId && ls.subject === first.subject
        ).length;
        if (sameSubjDay >= 1) {
            // Перевіряємо чи є інші дні де можна поставити
            const freeDays = countFreeDays(first.teacherId);
            const totalNeeded = state.workload
                .filter(w => w.classId === first.classId && w.subject.toLowerCase() === first.subject.toLowerCase())
                .reduce((sum, w) => sum + Math.ceil(parseFloat(w.hours)), 0);
            if (totalNeeded <= freeDays) return false; // є куди поставити — не дублюємо
        }
    }
...
function scoreSlot(task, first, d, s, schedule) {
    // Подвійна перевірка: якщо слот не valid — повертаємо Infinity
    // (це страховка, greedyPlace вже перевіряє isHardValid)
